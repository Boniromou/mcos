      ...
      class Base
        class << self
          ...
          def generate_signature(job)
            Digest::SHA256.hexdigest(canonical_job(job).join("\n"))
          end

          protected

          def canonical_job(job)
            job.to_h.slice(*signature_keys).inject([]) do |h, (k, v)|
              h << [k.to_s.downcase, v]
        ...
        def initialize(id:, worker:, operation: default_operation, runner:, auto_start: true, **opts)
          @id = id
